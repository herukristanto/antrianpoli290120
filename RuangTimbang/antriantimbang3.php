...
<head>
  ...
  body {
    background-image: url('Images/Display antrian ruang timbang Poli 3 - on.jpg');
...
if ($poli=="POLIKLINIK 3") {
  ...
  $query = "SELECT nomor_antrian FROM V_QP7ANTRIAN WHERE tgl_antrian='".$datenow."' AND status <> 0 AND kode like '28%' AND jam_pasien_timbang_selesai is null AND Nama_Poli LIKE '%".$poli."' order by kode, id";
  $params = array();
  $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
  $sql = sqlsrv_query( $conn, $query , $params, $options );
  if ($sql) {
  ...
  $query = "SELECT nomor_antrian FROM V_QP7ANTRIAN WHERE tgl_antrian='".$datenow."' AND status <> 0 AND kode like '34%' AND jam_pasien_timbang_selesai is null AND Nama_Poli LIKE '%".$poli."' order by kode, id";
  $params = array();
  $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
  $sql = sqlsrv_query( $conn, $query , $params, $options );
  if ($sql) {
...
<body onload='updatedata();'>
  ...
  <script>
    ...
    if (poli=='POLIKLINIK 3') {
      ...
      if (poli=='POLIKLINIK 7') {
        window.location.href="antriantimbang.php";
      } else if (poli=='POLIKLINIK 1') {
