...
<head>
  ...
  body{
    background-image: url('Images/Display antrian kamar dokterPoli 4 - on.jpg');
...
<!-- <body id="myBody"> -->
  <body id="myBody" onload="updatedata()">
    ...
    if ($row_count == 0) {
      ...
      $query = "SELECT nomor_antrian FROM qp7antrianaa where Id_Ruang = '".$idruang."' AND nomor_antrian like'".$kode."%' AND flage='y' ORDER BY Urutan ASC";
      $params = array();
      $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
      $sql = sqlsrv_query( $conn, $query , $params, $options );
      $row_count = sqlsrv_num_rows( $sql );
      ...
      $query = "SELECT nomor_antrian FROM qp7antrianaa where Id_Ruang = '".$idruang."' AND nomor_antrian like'".$kode."%' AND flage='x' ORDER BY Urutan ASC";
      $params = array();
      $options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
      $sql = sqlsrv_query( $conn, $query , $params, $options );
      $row_count = sqlsrv_num_rows( $sql );
      ...
      function updatedata() {
        ...
        if (status!=0) {
          ...
          if (nomoremg=='y' || nomoremg=='x') {
            window.location.href="ubahflageop.php?idruang="+idruang+"&id="+id;
          }
        ...
        if (status=='0') {
          window.location.href="antrian1offsc.php";
        } else if (status=='1') {
          window.location.href="antrian1sc.php?idruang="+idruang;
        } else if (status=='2') {
          window.location.href="antrian1blinksc.php?idruang="+idruang;
        }
      }
    </script>
